...
// THIS SOFTWARE IS A MODIFIED VERSION OF RLIMGUI BY Jeffery Myers
using System;
...
namespace rlImGui_cs
{
    public static class rlImGui
    {
        ...
        {
            Performance.PushTask("Render Triangles");
            if (count < 3)
                return;
        ...
        {
            Performance.PushTask("Imgui Renderer Setup");
            ...
            Performance.PushTask("Command Lists");
            
            for (int l = 0; l < data.CmdListsCount; l++)
            {
                ImDrawListPtr commandList = data.CmdLists[l];

                Performance.PushTask($"Command list {commandList._OwnerName.ToString()}");
                
                Rlgl.EnableScissorTest();
                
                for (int cmdIndex = 0; cmdIndex < commandList.CmdBuffer.Size; cmdIndex++)
                {
                    Performance.PushTask($"Command {cmdIndex}");
                    var cmd = commandList.CmdBuffer[cmdIndex];

                    Performance.PushTask("Scissor");
                    ...
                    Performance.PopTask();
                    Performance.PushTask("User Callback");
                    ...
                    Performance.PushTask("Draw Render Btach Active");
                    Rlgl.DrawRenderBatchActive();
                    Performance.PopTask();
            ...
            Performance.PushTask("Imgui Render Cleanup");
            Rlgl.SetTexture(0);
